        ...
        <div class="row">
            <div class="col-md-12 top-nav mb-3">
                <ul class="nav nav-tabsa custom-tabsa border-0 bg-white rounded overflow-hidden shadow-sm p-2 c-t-order"
                    ...
                    <li class="nav-item" role="presentation">
                        <a class="nav-link border-0 text-dark py-3 active" href="{{url('my_order')}}">
...
<script type="text/javascript">
    ...
    var intransitorsersref = database.collection('orders').where('id', "==", orderId);
    ...
    async function getOrderDetails() {
        ...
        intransitorsersref.get().then(async function (intransitorderSnapshots) {


            var orderDetails = intransitorderSnapshots.docs[0].data();
            ...
            if (orderDetails.paymentMethod.toLowerCase() == "razorpay") {
                ...
                payment_method = '<img alt="image" width="50%" src="' + image + '" >';
            ...
            var order_subtotal = order_shipping = order_total = order_number = 0;
            ...
            if (discount == null || discount == NaN || discount == "") {
            ...
            else if (discountType == "Fix Price") {
                ...
                for (let i = 0; i < orderDetails.products.length; i++) {
                    ...
                    var price = parseFloat(orderDetails.products[i]['discountPrice']) * parseFloat(orderDetails.products[i]['quantity']);
